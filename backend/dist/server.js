...
const asGrade = (v) => {
    const n = Number(v);
    return [10, 9.5, 9, 8].includes(n) ? n : 10;
};
app.get("/health", (_req, res) => res.json({ ok: true, mode: process.env.DATA_MODE || "mock", cards: db.cards().length }));
// ---- catalog ----
...
app.get("/search", (req, res) => res.json(db.searchCards(String(req.query.q ?? ""), req.query.lang || "All")));
