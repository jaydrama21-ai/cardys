...
import { shinyPricingTargets } from "../shiny.js";
import { setComp } from "../priceCache.js";
import { pgAvailable, initSchema, savePriceComp, savePriceHistoryPoint } from "../store/pg.js";
...
const centsToUsd = (v) => typeof v === "number" && v > 0 ? Math.round(v / 100) : null;
export async function fetchPriceChartingById(pcId) {
    const token = process.env.PRICECHARTING_API_TOKEN;
    if (!token)
        return null;
    try {
        const r = await fetch(`https://www.pricecharting.com/api/product?t=${token}&id=${encodeURIComponent(pcId)}`);
        if (!r.ok)
            return null;
...
export function compRowsFromPriceCharting(cardId, product) {
    const rows = [];
    const comps = typeof product["sales-volume"] === "number" ? product["sales-volume"] : 0;
...
export function matchCatalogCard(row, cards) {
    const name = row.name.toLowerCase();
    const set = row.set.toLowerCase();
    return (cards.find((c) => c.name.toLowerCase() === name && c.set.toLowerCase() === set) ??
    ...
    for (const t of targets) {
        ...
        if (raw) {
            if (card) {
                ...
                if (pgAvailable())
                    await savePriceHistoryPoint(cardId, "raw", raw.value);
            }
            if (t.valuePerUnit > 0)
                diffs.push(Math.abs(raw.value - t.valuePerUnit) / t.valuePerUnit);
        }
        const p10 = rows.find((r) => r.kind === "psa10");
        if (p10 && card)
            card.psa10 = p10.value;
        updated++;
