...
function defaultCsvPath() {
    if (process.env.SHINY_CSV && existsSync(process.env.SHINY_CSV))
        return process.env.SHINY_CSV;
...
export function loadShinyRows(csvPath) {
    ...
    const iName = col("product_name"), iSet = col("set_name"), iRarity = col("rarity"), iQty = col("quantity"), iVal = col("value_per_unit"), iPaid = col("paid_per_unit"), iTcg = col("tcgplayer_id"), iPc = col("pricecharting_id"), iGt = col("grade_type"), iGs = col("grade_subtype");
    const rows = [];
    for (const line of lines.slice(1)) {
        const f = parseLine(line);
        if (f.length < header.length - 1)
            continue;
        const idOf = (i) => (f[i] && f[i] !== "null" ? f[i] : null);
        rows.push({
            name: f[iName] ?? "",
            set: f[iSet] ?? "",
            rarity: f[iRarity] ?? "",
...
export function shinyPricingTargets(csvPath) {
